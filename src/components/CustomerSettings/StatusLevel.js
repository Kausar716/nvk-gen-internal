   ...
   active:[],inactive:[]
    ...
    onDragEnd = result => {
        ...
        if (source.droppableId === destination.droppableId) {
            ...
            this.props.customerStatusSort(this.state.inactive[result.source.index].id,this.state.inactive[result.destination.index].id,"down")
            else  this.props.customerStatusSort(this.state.inactive[result.source.index].id,this.state.inactive[result.destination.index].id,"up")
            this.setState({inactive:items});
            }else{
              
                        //        if(evt.willInsertAfter ==true)
            if(result.destination.index> result.source.index)
            this.props.customerStatusSort(this.state.active[result.source.index].id,this.state.active[result.destination.index].id,"down")
            else  this.props.customerStatusSort(this.state.active[result.source.index].id,this.state.active[result.destination.index].id,"up")
            ...
            console.log(task)
                      if(task.length > 0){
                        task[0].status =parseInt(task[0].status )==1? 0:1
                                    this.props.updateCustomerStatusLevelSettings(task[0].id,task[0]).then(data=>{
                                        this.props.getAllStatusMethods().then(()=>{
                                            // alert("ji")
                                            this.getCatgoryData()
                                        })
        ...
        onDeleteConfirm=(id)=>{
            ...
            result.then(res=>{
                ...
                this.setState({deleteon:false})
                   ...
                   confirmAlert({
                   title: 'Alert',
                   message: 'Please note that this Status Level is associated with Customer.Please reassign before deleting ',
                   buttons: [
                     {
                       label: 'Ok'
                     }
                                            ...
                                            >
                                                {this.state.inactive.map((item, index) => (
                                                    <Draggable
                                                        ...
                                                        <div className={this.state.isEditing===false  ? "a" :this.state.selectedID === item.id ? "reasonBackground a" : "a"}><span id={item.id}    >{item.status_level}</span>
